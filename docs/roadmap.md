...
File formats are versioned. A recipe writes a `larmor_recipe_version` field, and when a file written by a newer version is opened, unrecognized fields are dropped with a note rather than a refusal to load. A project bundle (`.larproj.json`, schema v2 with a v1 → v2 migration through the same path the recipe schema uses) saves the whole session in one file: every 1D workspace with its processed spectrum and overlays, 2D maps by reference with their recorded processing operations and display settings, figures kept from the Plotting studio, and batch-fit sessions with their settings and results.

The weak points are engineering and onboarding. The main window is a facade over eleven mixin modules (`larmor/desktop/mw_*.py`), so a menu action and its slot now sit in the same file, but nothing outside the test suite enforces the layout. There is no continuous integration; the test suite is substantial but runs only when started by hand, in one development environment. Releases so far are git tags: no installer executable has been built, verified end to end, and published, and nothing is signed. Development and testing happen on Windows only. Seven tutorials cover a first fit, constraints, figures, batch fitting, MQMAS, error analysis and a static wideline walkthrough; the first three, the MQMAS and the error-analysis ones run against the bundled example data (one glass: ¹¹B and ²⁷Al 1D spectra plus a ²⁷Al 3QMAS), while the batch and wideline ones depend on datasets that are described but not shipped.

## Next

- Put continuous integration in place, so the test suite runs on every push in a clean environment (the main-window split it was waiting on is done).
...
- Bring the documentation up to the app: example datasets beyond the single glass — a composition series and a relaxation set — so that every tutorial can be run from the repository alone.
...
Interaction. Keyboard-driven fitting: shortcuts to nudge the selected parameter, cycle sites, and start a fit. Automatic spinning-sideband detection that offers to add a linked sideband manifold in one click. An interactive dmfit amplitude calibration — today the import and export amplitude factors are fixed constants calibrated on one known fit, and pasting a single dmfit amplitude for a known line would pin the conversion exactly.
