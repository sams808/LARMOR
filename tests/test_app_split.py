...
GOLDEN_MENU = (
    ...
    (('&File',), 'Open &sample…  (list all its spectra)', 'Ctrl+Shift+S', False, False),
    ...
    (('&File',), '&Save recipe', 'Ctrl+S', False, False),
    (('&File',), 'Save fit &as…  (txt / csv / json / dmfit)', 'Ctrl+Shift+E', False, False),
    (('&File',), 'Save s&pectrum as…  (CSV, reopenable in LARMOR)', '', False, False),
    (('&File',), '&Copy plot to clipboard  (with all lines)', 'Ctrl+Shift+C', False, False),
    (('&File',), 'Save plot &image…  (png / svg)', '', False, False),
    (('&File',), 'Figure…', '', False, False),
    (('&File',), 'E&xit', '', False, False),
    ((), '&Process', '', False, True),
    (('&Process',), '&Experiment parameters… (νrot, B0, nucleus)', '', False, False),
    (('&Process',), 'Show processing panel', '', False, False),
    (('&Process',), '&FID ⇄ spectrum  (time ↔ frequency, re-apodize)', 'Ctrl+T', True, False),
    (('&Process',), 'Display &channel', '', False, True),
    (('&Process', 'Display &channel'), '&Real', '', True, False),
    (('&Process', 'Display &channel'), '&Imaginary  (inspect while phasing)', '', True, False),
    (('&Process', 'Display &channel'), '&Magnitude  |S|  (display only — the pipeline op is the panel checkbox)', '', True, False),
    (('&Process', 'Display &channel'), 'C&ycle channel  (real → imag → |S|)', 'Ctrl+I', False, False),
    (('&Process',), 'Processing s&teps…  (remove a step)', '', False, False),
    (('&Process',), 'Autophase (ACME)', '', False, False),
    (('&Process',), '&Drag to phase  (← → p0, ↑ ↓ p1 about the pivot)', 'Ctrl+P', False, False),
    (('&Process',), '2-point background…  (pick two flat points)', '', False, False),
    (('&Process',), 'Baseline auto (order 3)', '', False, False),
    (('&Process',), 'Baseline iterative (dead-time; Yon 2020)…', '', False, False),
    (('&Process',), 'Subtract &averages  (offset from the edges)', '', False, False),
    (('&Process',), 'Reset to original', '', False, False),
    (('&Process',), '&Calibrate axis…  (click a peak, set its ppm)', '', False, False),
    (('&Process',), '&Measure Δ (ppm / Hz)', '', True, False),
    (('&Process',), 'Subtract a spectrum (&background)…', '', False, False),
    (('&Process',), '&WURST excitation profile…  (divide out the sweep)', '', False, False),
    ((), '&Decomposition', '', False, True),
    (('&Decomposition',), '&New fit (clear lines)', '', False, False),
    (('&Decomposition',), 'Add &line  (pick a model)', '', False, True),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Gauss/Lorentz', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Gauss/Lorentz (area)', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'J-multiplet', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Spinning sidebands', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Voigt (true)', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Two-site exchange  (Bloch–McConnell)', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Czjzek (quad. distribution)', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'ext. Czjzek', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Czjzek, general d  (GIM at d = 5)', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Czjzek + δiso–C_Q correlation', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Amorphous (Gaussian Cq/eta dist.)', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Quad CT (2nd order)', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Quad 1st order (satellites)', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Quad CT + CSA', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'CSA distribution (disordered)', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'CSA powder (MAS/static)', '', True, False),
    (('&Decomposition', 'Add &line  (pick a model)'), 'Function fit', '', True, False),
    (('&Decomposition',), '&Apply recipe  (a recent fit → this data)', '', False, True),
    (('&Decomposition',), 'Add a line at every &peak…  (auto peak-pick)', '', False, False),
    (('&Decomposition',), '&Label lines from literature ranges  (Al[4], BO3, …)', '', False, False),
    (('&Decomposition',), 'Add spinning &sidebands…  (of a fitted line)', '', False, False),
    (('&Decomposition',), '&Detect spinning sidebands  (find the ±νrot repeat)', 'Ctrl+Shift+D', False, False),
    (('&Decomposition',), 'Add f&unction line…  (y = f(x; a,b,c,d))', '', False, False),
    (('&Decomposition',), 'Add background &spectrum…  (fit another spectrum)', '', False, False),
    (('&Decomposition',), 'Add a &copy of this spectrum…  (shifted — satellite / sideband manifold)', '', False, False),
    (('&Decomposition',), 'Add fit &zone', '', False, False),
    (('&Decomposition',), 'Clear zones', '', False, False),
    (('&Decomposition',), '&Simulate  (recompute the model)', 'F9', False, False),
    (('&Decomposition',), '&Fit', 'F5', False, False),
    (('&Decomposition',), '&Auto Fit (multi-start)…', '', False, False),
    (('&Decomposition',), '&Report (quantify)', 'F6', False, False),
    (('&Decomposition',), 'Fit &health details…  (why this verdict)', 'F7', False, False),
    (('&Decomposition',), '&Errors Analysis (χ² profile)…', '', False, False),
    (('&Decomposition',), 'Monte-&Carlo errors…  (synthetic-noise refits)', '', False, False),
    (('&Decomposition',), 'Parameter correlations…  (from the last fit)', '', False, False),
    (('&Decomposition',), 'Compare with a saved fit…  (parameter diff)', '', False, False),
    (('&Decomposition',), 'Czjzek distribution P(C_Q)…  (what σ stands for)', '', False, False),
    (('&Decomposition',), 'χ² map (parameter pair)…  (is the pair determined?)', '', False, False),
    (('&Decomposition',), 'Ad&vanced', '', False, True),
    (('&Decomposition', 'Ad&vanced'), 'Co-&fit datasets…  (shared model, 1D + MQMAS)', '', False, False),
    (('&Decomposition', 'Ad&vanced'), 'Computing &parameters…  (kernel resolution)', '', False, False),
    (('&Decomposition', 'Ad&vanced'), 'Fit completion &threshold…  (Δσ % to stop)', '', False, False),
    (('&Decomposition', 'Ad&vanced'), 'Restrict around current values…  (glass protocol, Edén 2023)', '', False, False),
    (('&Decomposition', 'Ad&vanced'), 'Save &constraints as…  (reusable link/bound set)', '', False, False),
    (('&Decomposition', 'Ad&vanced'), 'Apply saved co&nstraints…', '', False, False),
    (('&Decomposition', 'Ad&vanced'), 'MQMAS F1 &reference…  (isotropic-axis align)', '', False, False),
    (('&Decomposition', 'Ad&vanced'), 'Predict at another &field…  (what at X T?)', '', False, False),
    ((), '&View', '', False, True),
    ...
    (('&View',), '&Literature shift ranges  (Edén 2023)', '', True, False),
    (('&View',), '&Offer spinning-sideband detection on load', '', True, False),
    ...
    (('&View',), '&Back to 2D map', 'Ctrl+2', False, False),
    (('&View',), 'Zoom to sites', '', False, False),
    (('&View',), 'Full spectrum', '', False, False),
    ...
    (('&Tools',), 'E&xperimental section…  (paragraph + Table S1 from acqus / procs / title)', '', False, False),
    ...
    (('&Tools',), 'Per-site relaxation…  (uses the current fit)', '', False, False),
    (('&Tools',), 'QCPMG (echo train → spectrum)…', '', False, False),
    (('&Tools',), 'Stitch frequency-stepped (&VOCS) spectra…', '', False, False),
    (('&Tools',), '&Herzfeld–Berger sideband analysis (ζ, η)…  (CSA from sideband intensities)', '', False, False),
    (('&Tools',), 'Referencing a&udit…  (SR of a session against its ¹H adamantane reference)', '', False, False),
    (('&Tools',), '&Read static pattern (C_Q, η)…  (three markers, no fit)', '', False, False),
    (('&Tools',), 'QCPMG: infinite-field δiso (2 fields)…', '', False, False),
    (('&Tools',), 'QCPMG: batch infinite-field δiso…', '', False, False),
    (('&Tools',), 'Variable temperature (Arrhenius / VFT)…', '', False, False),
    (('&Tools',), 'REDOR (dipolar coupling)…', '', False, False),
    (('&Tools',), 'Import DFT tensors (.magres)…', '', False, False),
    (('&Tools',), '2D MQMAS viewer/fit…', '', False, False),
    (('&Tools',), 'Multi-dataset fit (CLI): larmor multifit a.json b.json', '', False, False),
    (('&Tools',), '&NMR table…  (Larmor frequencies)', '', False, False),
    (('&Tools',), '&Conversion tools…  (shift / Cq / dipolar)', '', False, False),
    ((), '&Plotting', '', False, True),
    (('&Plotting',), 'Plotting &studio…  (build any figure)', '', False, False),
    (('&Plotting',), 'Plot &current spectrum…', '', False, False),
    (('&Plotting',), 'New &2D contour plot…', '', False, False),
    ((), '&Help', '', False, True),
    (('&Help',), '&Command palette…  (find any menu entry)', 'Ctrl+Shift+P', False, False),
